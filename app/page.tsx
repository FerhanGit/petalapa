  ...
  return (
    <main className="exact-home pixel-home sharp-home">
      ...
      <section className="pixel-how" id="how">
        <div className="pixel-inner">
          ...
          <div className="pixel-step-grid">
            {[[I.pen,'Избираш дизайн','Разгледай нашите колекции и избери своя стил.'],[I.profile,'Добавяш информация','Име, телефон, важни данни за твоя любимец.'],[I.cart,'Поръчваш','Потвърди поръчката си и ние изработваме твоя таг.'],[I.heart,'Повече сигурност','Готово! Твоят любимец е с теб — винаги.']].map(([icon,title,text],index)=><article className="pixel-step" key={title}><span className="pixel-step-num">{index+1}</span><span className="pixel-step-icon"><Ic d={icon} size={21}/></span><div><b>{title}</b><p>{text}</p></div></article>)}
          </div>
        </div>
      </section>
      <section className="pixel-designs" id="designs">
        <div className="pixel-inner">
          ...
          <div className="pixel-design-row">
            <div className="pixel-tag-grid">{DESIGNS.map(([variant,label])=><a className="pixel-tag-card" key={variant} href={`/order?style=${variant==='blush'?'floral':variant==='paw'?'minimal':variant==='amber'?'wood':variant}`}><div className="pixel-tag-stage"><ReferenceTag variant={variant} size={92} photo/></div><b>{label}</b></a>)}</div>
            <aside className="pixel-custom-card"><span>БЕСТСЕЛЪР</span><h3>Персонализиран<br/>по твой вкус</h3><p>Цвят, цветя, блясък, символи. Направи го уникален, както е и той.</p><a href="/order?style=custom">Разгледай всички <Ic d={I.arrow} size={13}/></a></aside>
          </div>
        </div>
      </section>
      <section className="pixel-security sharp-security" id="about">
        ...
        <div className="pixel-security-copy"><h2>Повече сигурност.<br/>По-малко тревоги.</h2><p>С NFC и QR технологията всеки, който намери твоя любимец, може бързо и лесно да се свърже с теб.</p><div className="pixel-security-points"><div><span><Ic d={I.lock} size={15}/></span><small>Твоите данни<br/>са защитени</small></div><div><span><Ic d={I.info} size={15}/></span><small>Актуална информация<br/>при намиране</small></div><div><span><Ic d={I.profile} size={15}/></span><small>Повече шанс<br/>за бързо завръщане у дома</small></div></div></div>
      </section>
    </main>
  );
}
