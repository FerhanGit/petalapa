...
const LAGOTTO = 'https://images.pexels.com/photos/15222727/pexels-photo-15222727.jpeg?auto=compress&cs=tinysrgb&w=1200';
...
export default function PetProfile({ tag, scanId }: { tag: PublicTag; scanId: number | null }) {
  ...
  return (
    <main className="pet-profile-shell">
      ...
      <article className="pet-profile-card">
        <div className="pet-profile-photo">
          <img src={tag.photo_url || LAGOTTO} alt={tag.name ?? 'Домашен любимец'} />
          <div className="pet-profile-tag"><PhotoTag s={forest} size={66} qr={false} nfc={false} /></div>
        ...
        <div className="pet-profile-body">
          ...
          <section className={`pet-owner-box${lost ? ' is-lost' : ''}`}>
            <span className="owner-label">СТОПАНИН</span>
            <h2>{ownerName}</h2>
            {lost && <p className="pet-lost-copy">Ако сте намерили {tag.name || 'този любимец'}, моля свържете се със стопанина.</p>}
            <div className="pet-owner-actions">
              <a className={`btn btn-primary${lost ? ' btn-lost' : ''}`} href={tel} aria-disabled={!tel}><Ic d={I.phone} size={17} /> {primary?.phone ? `Обади се: ${primary.phone}` : 'Няма телефон'}</a>
              <a className="btn btn-outline" href={sms} aria-disabled={!sms}><Ic d={I.share} size={17} /> Изпрати местоположение</a>
            </div>
          </section>
